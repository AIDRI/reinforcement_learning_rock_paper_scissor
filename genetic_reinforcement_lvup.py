...
def human_(opposition, strategies=strategies, rounds=20): #functiun to play : opposition = human, we called the function to ask the move to the player, strategies is my list of strategies
    ...
    bot = [], player = [], cum_score = 0 #initialize history move (bot and player) and the end score
    ...
    for trial in range(rounds): #repeat the loop 20 times
        ...
        player_m = opposition(player, bot) #asking the move of the player : oppostion() called the function human()
...
if __name__ == '__main__':
    def human(bot, player): #it is the function to ask the player to move.
        return input(f'Choose one of {rps!r}: ') #we return him choice between R (rock) P (paper) S (scissors)
